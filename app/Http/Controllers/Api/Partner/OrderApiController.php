...
class OrderApiController extends Controller
{
    ...
    public function fetchOrdersFromTikTok(string $shopId, Request $request): array
    {
        ...
        // Only add order_status if it's provided and not 'ALL'
        if ($request->has('status') && $request->get('status') !== 'ALL') {
            $bodyData['order_status'] = $request->get('status');
        }

        // Only add buyer_user_id if provided
        if ($request->has('buyer_user_id') && $request->get('buyer_user_id')) {
            $bodyData['buyer_user_id'] = $request->get('buyer_user_id');
        }

        // Only add warehouse_ids if provided and not empty
        if ($request->has('warehouse_ids') && !empty($request->get('warehouse_ids'))) {
            $bodyData['warehouse_ids'] = $request->get('warehouse_ids');
        }
        ...
        if (!$response->successful()) {
            Log::error('TikTok Orders API failed', [
        ...
        $data = $response->json();
        Log::info('TikTok Orders API response', [
